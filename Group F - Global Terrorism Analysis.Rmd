---
title: "Assignment - GROUP F"
author: Fergus Buckley, Javier Contreras, Ryan Hayashino, Karim El Shammaa, Richard
  Rohla, Cedric Viret, Laurens Doedes Breuning ten Cate
output:
  html_document:
    number_sections: no
    theme: united
...
```{r}
setwd("~/IE-BIGDATA/Term 1/R-statistics/Assignment/Final Assignment - Group F")
...
It seems that there is a medium correlation (r=0.64, p<0.05) for world population and terrorist attack counts. However, they seem to disconnect when the population reaches more than 5.8  billion. 
...
Lets see if we can make a better fit with higher order versions of the population variable. 
```{r}
...
```{r}
lin2 <- qplot(predict(m2), rstandard(m2), geom="point") + geom_hline(yintercept=0, colour=I("blue"), alpha=I(0.5))
lin3 <- qplot(predict(m3), rstandard(m3), geom="point") + geom_hline(yintercept=0, colour=I("blue"), alpha=I(0.5))
...
*model1: Ajusted R-squared of 0.57
*model2: Ajusted R-squared of 0.50
*model3: Ajusted R-squared of 0.79
...
\large
\left\{ \begin{eqnarray*}
H_0: &\mu_{ISIL} = \mu_{Taliban} = \mu_{Boko Haram} = \mu_{SL} = \mu_{LTTE} \\
...
All of the weapon_type comparisson's are one-on-one significantly different EXCEPT for:
* Melee-Explosives/Bombs/Dynamite 
* Melee-Firearms
...
##6.6 Interactive overview of Rules
```{r}
inspectDT(terror_rules)

plotly_arules(terror_rules)
```
